...
export default function TeamsPage() {
  ...
  const reviewJoinRequest = async (id: string, action: 'approve' | 'reject') => {
    setBusy(true);
    try {
      ...
      const d = await res.json().catch(() => ({}));
      if (res.ok) {
        setMsg({ type: 'ok', text: action === 'approve' ? '已同意入队申请' : '已拒绝入队申请' });
        reload();
      } else {
        setMsg({ type: 'err', text: d.error || '审核失败' });
      }
    } finally { setBusy(false); }
  };
              ...
              <div key={team.id} className="bg-white rounded-2xl border border-gray-200/80 p-5 hover:border-gray-300 transition">
                ...
                {isLeader && team.joinRequests?.length > 0 && (
                  <div className="mt-3 p-3 bg-amber-50 rounded-xl border border-amber-100">
                    <h4 className="text-xs font-semibold text-amber-700 mb-2">待审核入队申请</h4>
                    <div className="space-y-2">
                      {team.joinRequests.map((req: any) => (
                        <div key={req.id} className="flex flex-wrap items-center justify-between gap-2 bg-white/70 rounded-lg px-3 py-2">
                          <div>
                            <p className="text-xs font-medium text-gray-800">{req.user?.name || '-'}</p>
                            <p className="text-[11px] text-gray-400">{req.user?.school || req.user?.email || '-'}</p>
                            {req.message && <p className="text-[11px] text-amber-700 mt-0.5">留言：{req.message}</p>}
                          </div>
                          <div className="flex gap-1">
                            <button onClick={() => reviewJoinRequest(req.id, 'approve')} disabled={busy}
                              className="px-2 py-1 text-[11px] bg-green-50 text-green-600 rounded-lg disabled:opacity-50">同意</button>
                            <button onClick={() => reviewJoinRequest(req.id, 'reject')} disabled={busy}
                              className="px-2 py-1 text-[11px] bg-red-50 text-red-600 rounded-lg disabled:opacity-50">拒绝</button>
                          </div>
                        </div>
                      ))}
                    </div>
